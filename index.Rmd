...
```{r}
 ...
 vec_common_name_cluster <-
  full_join(df_clustered,df_observaciones, by = c("common_name"= "Common.Name")) %>% 
  pivot_wider(names_from = observacion_mes, values_from = observation_count_mean_log10, names_prefix = "mes_") %>% 
  clean_names() %>% 
...
names(vec_common_name_cluster) <- c("common_name")
...
df_observaciones_clustered %>% 
  ...
  ggplot(aes(observacion_mes, common_name, fill = observation_count_mean_log10)) +
    geom_tile() +
